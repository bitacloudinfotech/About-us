"""
Build a neofetch-style info card SVG (Andrew6rant style) for Tushar Kashyap
to sit to the RIGHT of the ASCII portrait.
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Now", "Product Owner @ CIM Group"),
    ("kv", "Prev", "Sr. AI Data Engineer @ McSquared.ai"),
    ("kv", "Also", "Sr. Data Engineer @ FIFA (Stagwell)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Data", "Databricks, PySpark, SQL, Azure"),
    ("kv", "AI / LLM", "Agentic Workflows, RAG, FastAPI"),
    ("kv", "BI", "Power BI (PL-300), DAX, Semantics"),
    ("kv", "DevOps", "Azure, Docker, K8s, GitHub Actions"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Processed 500M+ records with PySpark"),
    ("bul", "Reduced runtime 8-9h → 1-2h (~75% opt.)"),
]
...
parts.append(
    f'<text x="{W/2:.1f}" y="{TITLEBAR_H/2 + 4:.1f}" fill="{MUTED}" font-size="12" '
    f'text-anchor="middle">tushar@github: ~$ neofetch</text>'
)
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (
            ...
            f'<tspan fill="{GREEN}">tushar</tspan><tspan fill="{MUTED}">@</tspan>'

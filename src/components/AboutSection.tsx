...
interface Pillar { icon: LucideIcon; label: string; sub: string; }
...
export default function AboutSection() {
  return (
    <section id="about" aria-labelledby="about-heading" className="section-padding"
      ...
      <div aria-hidden="true" style={{ position: 'absolute', inset: 0, backgroundImage: `linear-gradient(var(--border-subtle) 1px, transparent 1px), linear-gradient(90deg, var(--border-subtle) 1px, transparent 1px)`, backgroundSize: '64px 64px', opacity: 0.4, maskImage: 'radial-gradient(ellipse 70% 60% at 80% 50%, black 0%, transparent 100%)', WebkitMaskImage: 'radial-gradient(ellipse 70% 60% at 80% 50%, black 0%, transparent 100%)', pointerEvents: 'none' }} />
      <div className="container relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="section-badge w-fit reveal">About BITA CLOUD INFO TECH</div>
              <h2 id="about-heading" className="section-title reveal reveal-delay-1">Built by engineers,<br /><strong>for engineers.</strong></h2>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4 reveal reveal-delay-2">
              {pillars.map((p) => (
                <div key={p.label} className="grad-border-card p-5 space-y-3" style={{ background: 'var(--bg-card)' }}>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'var(--accent-cyan-dim)', border: '1px solid var(--border-strong)' }}>
            ...
            <div className="grad-border-card p-7 space-y-4 reveal reveal-delay-3" style={{ background: 'var(--bg-card)' }}>
              <div className="section-badge w-fit">Careers at BITA</div>
              <h3 className="font-bold text-xl" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>Join our global engineering team.</h3>
